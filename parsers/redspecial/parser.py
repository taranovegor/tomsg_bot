...
def replace_img(match):
    src = match.group(1)
    return f'<a href="https://redspecial.ru/{src.lstrip("/")}">🖼 Изображение</a>'
...
class Parser(BaseParser):
    ...
    def supports(self, url):
        return "redspecial.ru" in url and "#div_comment_" in url
    ...
    def parse(self, string: str) -> Content:
        ...
        return Content(
            author=Link(
                url=f"https://redspecial.ru/users/{comment['login']}/", text=comment["login"]
            ),
    ...
    def __parse_topic(self, url: str):
        ...
        if len(segments) < 3 or not url.startswith("https://redspecial.ru"):
            raise ParseError
        ...
        body = self.fetch(f"https://redspecial.ru/api_topics/{topic_id}")
    ...
    def fetch_comments(self, topic_id):
        return json.loads(
            self.fetch(f"https://redspecial.ru/api_noauth.php?action=comments&topic_id={topic_id}")
        )["comments"]
